...
class Get_Correlation(nn.Module):
    ...
    def forward(self, x):
        ...
        affinities = torch.einsum('bcthw,bctsd->bthwsd', x, torch.cat([x2[:,:,1:], x2[:,:,-1:]], 2))  # repeat the last frame
        affinities2 = torch.einsum('bcthw,bctsd->bthwsd', x, torch.cat([x2[:,:,:1], x2[:,:,:-1]], 2))  # repeat the first frame
        features = torch.einsum('bctsd,bthwsd->bcthw', torch.cat([x2[:,:,1:], x2[:,:,-1:]], 2), F.sigmoid(affinities)-0.5 )* self.weights2[0] + \
            torch.einsum('bctsd,bthwsd->bcthw', torch.cat([x2[:,:,:1], x2[:,:,:-1]], 2), F.sigmoid(affinities2)-0.5 ) * self.weights2[1]

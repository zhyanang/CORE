...
def seq_train(loader, model, optimizer, device, epoch_idx, recoder):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        Face = device.data_to_device(data[4])
        Rhand = device.data_to_device(data[5])
        Lhand = device.data_to_device(data[6])
        ret_dict = model(vid, vid_lgt, label=label, label_lgt=label_lgt, Face_keypoint=Face, Rhand_keypoint=Rhand, Lhand_keypoint=Lhand)

        # face_Rhand_value.append(ret_dict['face_Rhand'].item())
        # face_Lhand_value.append(ret_dict['face_Lhand'].item())
        # Rhand_Lhand_value.append(ret_dict['Rhand_Lhand'].item())
    ...
    # recoder.print_log('\tF-R_adj: {:.10f}.'.format(np.mean(face_Rhand_value)))
    # recoder.print_log('\tF-L_adj: {:.10f}.'.format(np.mean(face_Lhand_value)))
    # recoder.print_log('\tR-L_adj: {:.10f}.'.format(np.mean(Rhand_Lhand_value)))
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder,
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            ...
            # face_Lhand_value.append(ret_dict['face_Lhand'].item())
            # Rhand_Lhand_value.append(ret_dict['Rhand_Lhand'].item())
            # face_hand.append(ret_dict['face_hand'].item())
        total_info += [file_name.split("|")[0] for file_name in data[-1]]
        total_sent += ret_dict['recognized_sents']
    # recoder.print_log('\tF-R_adj: {:.10f}.'.format(np.mean(face_Rhand_value)))
    # recoder.print_log('\tF-L_adj: {:.10f}.'.format(np.mean(face_Lhand_value)))
    # recoder.print_log('\tR-L_adj: {:.10f}.'.format(np.mean(Rhand_Lhand_value)))
    # recoder.print_log('\tface-hand_adj: {:.10f}.'.format(np.mean(face_hand)))
